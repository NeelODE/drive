...
<head>
    ...
    <title>File Manager - Vercel Blob</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .badge { display: inline-block; padding: 4px 8px; background: rgba(255,255,255,0.2); border-radius: 4px; font-size: 11px; margin-left: 10px; }
        .breadcrumb { display: flex; gap: 8px; margin-top: 15px; flex-wrap: wrap; }
        .breadcrumb a { color: rgba(255,255,255,0.9); text-decoration: none; padding: 5px 10px; border-radius: 5px; }
        .breadcrumb a:hover { background: rgba(255,255,255,0.1); }
        .main-content { padding: 30px; }
        .actions { display: flex; gap: 15px; margin-bottom: 25px; flex-wrap: wrap; }
        .actions form { flex: 1; display: flex; gap: 10px; }
        .actions input { flex: 1; padding: 10px; border: 1px solid #ddd; border-radius: 5px; }
        .btn { padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; font-weight: 500; transition: 0.2s; }
        ...
        .file-actions .btn { padding: 6px 12px; font-size: 13px; }
...
<body>
    <div class="container">
        <div class="header">
            <h1>📁 File Manager <span class="badge">Vercel Blob</span></h1>
        ...
        <div class="main-content">
            <div class="actions">
                ...
                <form id="createFolderForm">
                    <input type="text" id="folder_name" placeholder="New folder name" required>
            ...
            <div class="empty-state" id="emptyState" style="display: none;">No files yet</div>
        </div>
    </div>
    ...
    <script>
        ...
        function showStatus(msg, type, timeout = 3000) {
            const status = document.getElementById('status');
            status.textContent = msg;
            status.className = `status ${type}`;
            if (timeout > 0) setTimeout(() => status.className = 'status', timeout);
        }
        ...
        function updateBreadcrumb() {
            ...
            if (currentDirectory) {
                ...
                parts.forEach((part, i) => {
                    path += (i > 0 ? '/' : '') + part;
                    breadcrumb.innerHTML += ` / <a href="#" data-path="${path}">${part}</a>`;
                });
            }
            breadcrumb.querySelectorAll('a').forEach(a => {
                a.onclick = (e) => { e.preventDefault(); loadFiles(a.dataset.path); };
            });
        }
                ...
                if (data.files.length === 0) {
                    ...
                    data.files.forEach(file => {
                        ...
                        li.innerHTML = `
                            ...
                            <div class="file-info">
                                ${file.is_dir ? 
                                    `<div class="file-name"><a href="#" onclick="event.preventDefault(); loadFiles('${file.path}')">${file.name}</a></div>` :
                                    `<div class="file-name">${file.name}</div>`
                                }
                                <div class="file-meta">${file.size}${file.modified ? ' • ' + new Date(file.modified).toLocaleString() : ''}</div>
                            </div>
                            <div class="file-actions">
                                ${!file.is_dir ? `<a href="${file.url}" download="${file.name}" class="btn btn-primary">Download</a>` : ''}
                                <button class="btn btn-danger" onclick="deleteItem('${file.path}', '${file.name}')">Delete</button>
                            </div>
                        `;
